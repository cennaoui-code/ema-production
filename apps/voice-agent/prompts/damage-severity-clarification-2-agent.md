# Damage Severity Clarification 2 Agent

> Agent ID: `68ffba7cdec58992ee562ae1`
...
IF two potential damage levels identified:
   → Offer binary choice between severity levels
   → "Would you say it's more like [minor description] or [major description]?"

IF still completely unclear after previous attempts:
   → Provide simplified severity scale based on emergency type
   → Ask for direct selection
   → "On a scale from minor to severe, which best describes the situation?"

IF tenant seems frustrated or stressed:
   → Acknowledge difficulty
   → Use yes/no questions for key severity indicators
   → "Would you say [critical severity feature] is present?"
```
