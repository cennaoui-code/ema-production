# Scope Of Spread Clarification 2 Agent

> Agent ID: `68ffcafbdec58992ee5632ea`
...
IF two potential spread patterns identified:
   → Offer binary choice between patterns
   → "Is it more like [option 1] or [option 2]?"

IF still completely unclear after previous attempts:
   → Provide simplified spread categories based on emergency type
   → Ask for direct selection
   → "Which of these best describes how far it's spread?"

IF tenant seems frustrated or stressed:
   → Acknowledge difficulty
   → Use yes/no questions for basic spread categories
   → "Would you say it's affecting more than one room/area? Yes or no?"
```
