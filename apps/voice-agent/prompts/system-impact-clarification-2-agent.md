# System Impact Clarification 2 Agent

> Agent ID: `68ffc2d5dec58992ee562ffd`
...
IF two potential system impacts identified:
   → Offer binary choice between impacts
   → "Is it affecting [system 1] or [system 2]?"

IF still completely unclear after previous attempts:
   → Provide simplified system list based on emergency type
   → Ask for direct selection
   → "Which of these systems seems to be affected?"

IF tenant seems frustrated or stressed:
   → Acknowledge difficulty
   → Use yes/no questions for top system impacts
   → "Would you say this is affecting [specific system] at all?"
```
