# Habitability Impact Clarification 2 Agent

> Agent ID: `68ffd35adec58992ee5636ce`
...
IF two potential habitability levels identified:
   → Offer binary choice between levels
   → "Would you say this situation is more like [higher impact] or [lower impact]?"

IF still completely unclear after previous attempts:
   → Provide simplified habitability impact list
   → Ask for direct selection
   → "Which of these best describes your current situation?"

IF tenant seems frustrated or stressed:
   → Acknowledge difficulty
   → Use yes/no questions for key habitability factors
   → "Can you still safely [essential function] in your home?"
```
