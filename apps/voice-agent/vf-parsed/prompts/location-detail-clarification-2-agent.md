# Location Detail Clarification 2 Agent

> Agent ID: `68ff8b96dec58992ee561628`
...
IF two potential emergency locations identified:
   → Offer binary choice between locations
   → "Is it more like [location 1] or [location 2]?"

IF still completely unclear after previous attempts:
   → Provide simplified location list
   → Ask for direct selection
   → "Which of these best describes where the emergency is happening?"

IF tenant seems frustrated or stressed:
   → Acknowledge difficulty
   → Use yes/no questions for top locations
   → "Would you say this is happening in [location] at all?"
```
