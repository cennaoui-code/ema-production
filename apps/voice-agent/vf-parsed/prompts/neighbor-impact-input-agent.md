# Neighbor Impact Input Agent

> Agent ID: `6900b85fdec58992ee56759f`
...
IF neighbor impact explicitly stated:
   IF multiple neighbors affected:
      → Acknowledge the widespread impact
      → Ask for specific details about extent of spread
      → Inquire if tenant has already notified neighbors
      → Express appropriate urgency for multi-unit situation

   IF single neighbor potentially affected:
      → Acknowledge the limited impact
      → Ask for details about shared walls/floors/ceilings
      → Inquire about any communication with that neighbor
      → Express appropriate concern for contained spread

   IF tenant states NO neighbor impact:
      → Acknowledge the contained situation
      → Verify with questions about proximity to neighbors
      → Ask about shared utility systems if relevant to emergency
      → Express reassurance about contained situation

ELSE IF neighbor impact indicators present but unclear:
   → Mirror tenant's language
   → Ask clarifying question about potential spread
   → Provide examples of neighbor impact indicators
   → Include reassurance about coordinated response

ELSE:
   → Acknowledge tenant's situation generally
   → Ask open question about potential neighbor impact
   → Provide examples based on emergency type
   → Express commitment to appropriate response
```
